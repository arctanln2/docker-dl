...
def train_epoch(train_loader, optimizer, criterion, lr_scheduler, model):
        model.train()
        ...
        lr_scheduler.step()
        train_loss_value = train_running_loss/ (len(train_dataset) )
        train_acc_value = train_running_acc/ (len(train_dataset) )

        return train_loss_value, train_acc_value

def valid_epoch(valid_loader, criterion, model):
    model.eval()
    ...
    valid_loss_value = valid_running_loss/ (len(valid_dataset))
    valid_acc_value = valid_running_acc/ (len(valid_dataset))
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(train_dataset, batch_size=256,
                               **kwargs)#, prefetch_factor=2, num_workers=4)
    ...
    model = Net().to(device)
    # print(local_rank, "Model created.", dist.get_rank(), )
    ...
    num_epochs = 30
    file.write(f"model initialization,{t2-t1}\n")
    t3=time()
    for epoch in range(num_epochs):
        ...
        train_loss_value, train_acc_value = train_epoch(train_loader, optimizer, criterion, lr_scheduler_values, model)
        t2=time()
        # print(local_rank,"train cost",t2-t1,"s")
        file.write(f"train {epoch},{t2-t1}\n")
        valid_loss_value, valid_acc_value = valid_epoch(valid_loader, criterion, model)    
        t3=time()
        # print(local_rank,"valid cost",t3-t2,"s")
        print("Train_Epoch: {}/{} Training_Loss: {} Training_acc: {:.2f}\
                   ".format(epoch, num_epochs-1, train_loss_value, train_acc_value))

        print("Valid_Epoch: {}/{} Valid_Loss: {} Valid_acc: {:.2f}\
                   ".format(epoch, num_epochs-1, valid_loss_value, valid_acc_value))
        file.write(f"epoch {epoch},{t3-t1}\n")
        print('--------------------------------')
    t1=time()
    file.write(f"total time,{t1-t0}\n")
    file.close()
    print("finished.")
